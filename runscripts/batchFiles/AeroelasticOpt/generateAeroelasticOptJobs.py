...
for linType in ["Linear", "Nonlinear"]:
    ...
    for level, meshSize in zip(levels, meshSizes):
        ...
        runCommand = f"python aeroStructRun-MultipointParallel.py --task opt --optType structMass --timeLimit {(runTime*3600 - 600)} --addStructDVs --flightPointSet maneuverOnly --aeroLevel {level} --structLevel 1 {linOption} --initDVs {initDVs} --output {outputDir}"
